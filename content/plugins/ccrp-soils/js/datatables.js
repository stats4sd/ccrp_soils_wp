...
/// Filter one table by selecting from another
/// Requires Params:
/// // sourceTable -> initialised dataTable;
/// // sourceCol -> string, will be seperated by '.' to allow for object data;
/// // targetTable -> initialised dataTable;
/// // targetCol -> int -> column number;
/// // exact (optional - default = true);
function linkTableByFilter(params) {

  // *** to impliment (maybe) - additive search for multiple selections;
  // currently, data[0] below means that only the first item in the selected indexes will be used for the search.
  var sourceTable = params.sourceTable;
  var targetTable = params.targetTable;
  var sourceCol = params.sourceCol;
  var targetCol = params.targetCol;

  if(sourceCol.search('.') > -1) {
    sourceCol = sourceCol.split('.');
  }
  else{
    sourceCol = [sourceCol];
  }

  //setup on.select function to filter the question groups table by selection:
  sourceTable.on('select',function(e,dt,type,indexes){
    if(type === 'row') {
      var data = sourceTable.rows(indexes).data()
      console.log("data",data);
      //pluck into data by sourceCol array:
      //
      sourceCol.forEach(function(item,index){
        data = data.pluck(item);
      })
      ...
      //set default search to "exact"
      var search = '(^|,)(';
      //add each item from data array to the search with 'or' statements:
      data.forEach(function(item,index){
        if(index > 0) {
          search += "|";
        }
      ...
      targetTable.column(targetCol).search(search,true,false).draw();
    }
  })

  sourceTable.on('deselect',function(e,dt,type,indexes){
    if(type == 'row'){
      var data = sourceTable.rows(indexes).data();
      ...
      targetTable.column(targetCol).search(newSearch,true,false).draw();
